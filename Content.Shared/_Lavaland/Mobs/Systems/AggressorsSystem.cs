...
namespace Content.Shared._Lavaland.Mobs.Systems;

[Virtual] public partial class AggressorsSystem : EntitySystem
{
    ...
    public override void Initialize()
    {
        ...
        SubscribeLocalEvent<AggressorsComponent, DamageChangedEvent>(OnDamageChanged);
        SubscribeLocalEvent<AggressorComponent, MobStateChangedEvent>(OnMobStateChange);
    }

    private void OnDamageChanged(Entity<AggressorsComponent> ent, ref DamageChangedEvent args)
    {
    ...
    {
        TryComp<AggressorsComponent>(uid, out var aggro);
        return aggro?.Aggressors ?? null;
    }

    public void RemoveAggressor(Entity<AggressorsComponent> ent, EntityUid aggressor)
    {
        if (ent.Comp.Aggressors.Contains(aggressor))
            ent.Comp.Aggressors.Remove(aggressor);
    }

    public void AddAggressor(Entity<AggressorsComponent> ent, EntityUid aggressor)
    {
        ent.Comp.Aggressors.Add(aggressor);

        var aggcomp = EnsureComp<AggressorComponent>(aggressor);

        if (!aggcomp.Aggrods.Contains(ent))
            aggcomp.Aggrods.Add(ent);
    }
    ...
    public void CleanAggressions(EntityUid aggressor)
    {
        ...
        foreach (var aggrod in aggcomp.Aggrods)
            if (TryComp<AggressorsComponent>(aggrod, out var aggressors))
